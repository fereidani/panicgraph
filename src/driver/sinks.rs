...
impl Sink {
    ...
    /// The category of the first panic the sink raises.
...
/// Functions that raise one kind of panic, by the last segment of their
/// path.
///
/// Third party collections carry their own allocation failure funnels, for
/// example hashbrown's `Fallibility::capacity_overflow`. Suppressing
/// allocation panics has to reach those too, otherwise every crate using a
/// hash map keeps reporting allocation noise.
const FUNNELS: &[(&str, Sink)] = &[
    ("capacity_overflow", unwind(Category::CapacityOverflow)),
    ("handle_alloc_error", abort(Category::AllocFailure)),
    ("alloc_err", abort(Category::AllocFailure)),
    ("oom", abort(Category::AllocFailure)),
    ("panic_arc_overflow", unwind(Category::RefCountOverflow)),
    ("panic_rc_overflow", unwind(Category::RefCountOverflow)),
    // Generated by the standard library's `ub_checks` macro.
    ("precondition_check", abort(Category::UbCheck)),
];

/// The crates the standard library is built from.
const LIBRARY: &[&str] = &["core", "alloc", "std"];
...
/// says nothing about what was discarded. The `Result` method that calls
/// it does, and matching its type is stable where matching a message is
/// not.
...
impl SinkTable {
    ...
    fn classify(tcx: TyCtxt<'_>, did: DefId) -> Option<Sink> {
        ...
        // Panics written in a readable funnel take its category where they
        // are recorded; an unreadable funnel is classified by its name.
        if !tcx.is_mir_available(did)
            && let Some(sink) = Self::funnel(tcx, did)
        {
            return Some(sink);
        }
    ...
    /// The panic a function raises when its name marks it as a funnel.
    ///
    /// Names count in the standard library and in dependencies. A local
    /// function named `oom` may index or unwrap, and filing those panics
    /// under a category suppressed by default would hide them. Only the
    /// standard library's macro generates `precondition_check`.
    pub fn funnel(tcx: TyCtxt<'_>, did: DefId) -> Option<Sink> {
        let leaf = tcx.opt_item_name(did)?;
        let &(_, sink) =
            FUNNELS.iter().find(|(name, _)| *name == leaf.as_str())?;
        let library = LIBRARY.contains(&tcx.crate_name(did.krate).as_str());
        let foreign = !did.is_local() && !sink.is_only(Category::UbCheck);
        (library || foreign).then_some(sink)
    }
    ...
    /// The error type `origin` discards, when it is an instance of
    /// `Result::unwrap` or `Result::expect`.
    pub fn discarded_error<'tcx>(
        tcx: TyCtxt<'tcx>,
        origin: ty::Instance<'tcx>,
    ) -> Option<ty::Ty<'tcx>> {
        let did = origin.def_id();
        if tcx.crate_name(did.krate).as_str() != "core" {
            return None;
        }
        if !matches!(
            Self::def_path(tcx, did).as_str(),
            "result::unwrap" | "result::expect"
        ) {
            return None;
        }
        origin.args.get(1)?.as_type()
    }

    /// Sharpens an unwrap by the error type it discards.
    ///
    /// Unwrapping a poisoned lock result is a poison panic, and unwrapping a
    /// formatting result is a formatting one.
